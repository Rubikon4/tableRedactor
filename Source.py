...
class SourceFrame(ttk.Frame):
    ...
    def build_ui(self):
        ...
        self.output_button = ttk.Button(self, text="Выбрать конечную папку", command=self.choose_output_folder)
        ...
        self.radio_once = ttk.Radiobutton(self, text="Единоразовая обработка",
                                          ...
                                          command=self.update_inputs_state)
        self.radio_once.grid(row=0, column=1, sticky="w", padx=5, pady=5)
        self.btn_choose_file = ttk.Button(self, text="Выбрать файл", command=self.choose_file)
        ...
        self.btn_choose_folder = ttk.Button(self, text="Выбрать папку", command=self.choose_folder)
        ...
        self.entry_template.bind("<Return>", self.validate_template)
    ...
    def validate_template(self, event=None):
        ...
        if "." in name:
            messagebox.showinfo("Ошибка шаблона", "Введите только имя файла без расширения, например: 'отчет', не 'отчет.xls'")
